...
//V»rifie si la liste est pleine ou non
...
//V»rifie si la liste est vide ou non
...
void getNextAsserOrdre() {
    // Mettre ? jour la position de Gali
    updatePosition();
...
inline void gestionnaireOrdres() {
    ...
    switch (listeAsser.ordres[listeAsser.begin].type) {
        ...
        case RECALAGE_1: //	Mettre une autre valeure que erreurTolerance si n»cessaire
            if (((ABS(distance.variation)) < 5) && ((ABS(angle.variation)) < 5) && ((millis - timer) > 1000))

                if (millis - timerRecalage > 20) {
                    //Mettre la consigne ? 0
                    distance.ordre = 0;
                    angle.ordre = 0;
                    angle.erreur = 0;
                    distance.erreur = 0;
    ...
    case RECALAGE_2: //	Mettre une autre valeur que erreurTolerance si n»cessaire
